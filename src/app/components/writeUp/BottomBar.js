...
export const BottomBar = ({id}) => {
  ...
  const fetchCommentsCount = async () => {
    const response = await fetch(API_URL+`writeup/comments/count?writeup=${id}`);
  ...
  const fetchLikes = async () => {
    const response = await fetch(API_URL+`writeup/impressions?writeup=${id}`, {
      method: "GET",
  ...
  const handleLike = async () => {
    ...
    await fetch(API_URL+`writeup/like?writeup=${id}`, {
      method: "GET",
      headers: toAuthHeaders({})
    });
  ...
  return (
    <div className="min-h-[10vh] p-5 flex flex-row justify-around">
      ...
      >
        <Eye />
        {views===false ? <Loader2 className="animate-spin" /> : num.toHumanString(views)}
      </Button>
      ...
      <CommentsDrawer wid={id} count={commentsCount} isOpen={showComments} setIsOpen={setShowComments} />
