...
const SymbolList = forwardRef((props, ref)=> {
    ...
    <ul className="p-2 flex flex-col border-1 rounded-md bg-default-50">
      {props.items.length
        ? props.items.map((item, index) => (
          <li
            className="hover:bg-default-100 rounded-md p-1 font-semibold text-emerald-500"
            key={index}
...
const Items = async (query) => {
  ...
  const symbolAutocomplete = async (text) => {
    const data = await fetch(API_URL+`symbols/search?query=${text}`, {
      method: "GET"
    });
  ...
  return symbolAutocomplete(query)
    .then((res)=>{
      if(res && res.length)
      {
        res.push({symbol: query});
        return res.filter((x)=>x.symbol.toLowerCase().indexOf(query.toLowerCase())!==-1);
      }
      else{
        return [{symbol: query}];
      }
...
export default {
  ...
  render: () => {
    ...
    return {
      onStart: props => {
        component = new ReactRenderer(SymbolList, {
          props,
          editor: props.editor,
        })
