...
export const WriteUpCard = ({writeup, setWriteUps}) => {
  ...
  return (
    <Card
      ...
      className={`w-full max-w-[330px] max-h-[400px] h-full bg-background rounded-md p-0 border-1`}
      isPressable
      onPress={()=>router.push(`/view/writeup/${writeup.id}`)}
      shadow="none"
    >
      <CardHeader
        className="flex flex-col justify-center h-[30vh] max-h-[300px] rounded-md p-0"
      >
        <Image
          fetchPriority="low"
          loading="lazy"
          alt="thumbnail"
          itemProp="thumbnail"
          className="object-fit aspect-[7/5]"
          radius="none"
          conn
